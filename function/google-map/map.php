...
function showAllColleges(allData) {
	...
	Array.prototype.forEach.call(allData, function(data){
		...
		strong.textContent = data.device_name+" " +" ( "+ km+" )";
		...
		marker.addListener('click', function(){
          window.location.href ='../../index.php?page=normal-ads-view-Item&adId='+data.id; 	
	    });
	})
}	
...
<body>
	...
	<div class="container">
	<br><br>
	   <a href="../../index.php?page=normal-ads-view-Item&adId=<?php echo $id; ?>" style="margin:0 15px; color:white; font-size:18px;">Back</a>
		<center><h1>To select similar phone ads from Google Map.</h1></center>
...
$data = $resultSet->fetch_all(MYSQLI_ASSOC);
		...
		public function getAllColleges($id,$model,$mType) {
		    
			$this->nn1 = $id;
			$this->nn2 = $model;
			$this->nn3 = $mType;
			$sql = "SELECT * FROM `phoneads` WHERE `Admin_Approval` ='1'  AND `id` != '$this->nn1'  AND `brand` ='$this->nn2' AND `device` = '$this->nn3'";
			$stmt = $this->conn->prepare($sql);
			$stmt->execute();
			$resultSet = $stmt->get_result();
            $data = $resultSet->fetch_all(MYSQLI_ASSOC);
			...
			if($count > 0){
			  return $data;
			    ...
			    $sql = "SELECT * FROM `phoneads` WHERE `Admin_Approval` ='1'  AND `id` != '$this->nn1' ";
			    $stmt = $this->conn->prepare($sql);
		      	$stmt->execute();
			    $resultSet = $stmt->get_result();
                $data = $resultSet->fetch_all(MYSQLI_ASSOC);
